...
interface HeroesState {
	...
	actions: {
		...
		updateHealth: (operation: "increment" | "decrement") => void;
		updateMorale: (operation: "increment" | "decrement") => void;
	};
}
	...
	persist(
		immer((set) => ({
			...
			actions: {
				...
				updateHealth: (operation) => {
					set(
						produce((state: HeroesState) => {
							if (!state.hero) return;
							if (operation === "increment") {
								state.hero.health.current = state.hero.health.current + 1;
							} else {
								state.hero.health.current = state.hero.health.current - 1;
							}
						}),
				...
				updateMorale: (operation) => {
					set(
						produce((state: HeroesState) => {
							if (!state.hero) return;
							if (operation === "increment") {
								state.hero.morale.current = state.hero.morale.current + 1;
							} else {
								state.hero.morale.current = state.hero.morale.current - 1;
							}
						}),
